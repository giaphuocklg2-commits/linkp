...
const { getUserTier, MEMBER_RULES } = require('../src/lib/membership.js');
...
async function test() {
  try {
    ...
    const response = await fetch(`${CONVERSIONS_URL}?type=${type}&page_size=${pageSize}`, {
      headers: { 'X-API-Key': API_KEY, 'Accept': 'application/json', 'User-Agent': 'LinkP-Admin/2.0' }
    });

    if (!response.ok) throw new Error(`AddLiveTag API HTTP ${response.status}`);
    ...
    for (const item of items) {
      ...
      const isCompleted = statusCode === 'completed' || statusCode === 'paid' || rawStatus === 'Hoàn thành' || rawStatus === 'Đã thanh toán' || commStatus === 'Chờ trả hoa hồng' || commStatus === 'Đã trả';
      ...
      if (existingRes.rows.length === 0) {
        ...
        `, [orderCode, resolvedUserId, resolvedUserName, item.item_name || 'Sản phẩm Shopee', item.affiliate || 'Shopee Mall', item.image || '', orderVal, comm, userCb, adminRev, 'PENDING', rawSub || 'app_direct', item.purchase_time ? Number(item.purchase_time) : Date.now() / 1000]);
        ...
        if (prev.status !== 'APPROVED' && targetStatus === 'APPROVED') {
          ...
          await query(`UPDATE public."AffiliateOrder" SET
              ...
              "adminRevenue" = $7
            WHERE id = $8
          `, [resolvedUserId, resolvedUserName, item.item_name || 'Sản phẩm Shopee', orderVal, comm, userCb, adminRev, prev.id]);
